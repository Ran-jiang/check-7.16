...
- 核查以法规**现行有效版本**为基准；文书引用行为时的历史版本导致的表述差异不做自动归因，前端固定提示用户自行核对行为时文本。
- 语义核查判定条号疑似错误时，会按"模型提名候选 → 取回原文验证（确定性包含或语义比对）→ 不符则携带反馈再提名"迭代复查，最多三轮；穷尽后在结论中列明已复查条号转人工。
- 同一法条的同一错误因简称/承前引用多处出现时合并为一张卡片，保留全部定位。
...
- 未支持：跨句复杂召回、完整案情事实归纳。
- 案例 MCP 返回候选但无法确定唯一同一案例时保留为 `manual_review`，并说明具体冲突原因。案例身份确认后，以法宝收录的裁判说理（"本院认为"节选）为基准逐观点句核查：说理切句编号，模型按句号定位依据，程序取回原句作为展示证据；观点在说理中无对应依据、或转述曲解说理原意，分别输出两类错误。不评价法律论证是否成立。
