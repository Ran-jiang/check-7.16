...
1. 逐字精确比对：默认开启，不对标点、空格或数字格式做归一化。
...
插件从 Word 读取当前完整 DOCX，调用同域的 FastAPI 服务，然后在右侧任务窗格展示逐字差异和语义核查结果。API Key 只保存在服务器 `.env`，不会发送到 Word 客户端。
...
- 已支持默认逐字差异输出和可选千问语义核查。
- 已支持无条号法规引用：对本地已有全文的法规召回最相关的当前有效条款，再执行语义核验；此时逐字比对标记为不适用。
...
- 语义审查只基于 claim、已查得的法条、元数据和逐字差异，不引入外部案件事实。
- 未支持：跨句复杂召回、完整案情事实归纳。
