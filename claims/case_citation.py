...
from .schema import CaseRef, CaseReferenceType
...
# 约束：
#   1. \S{2,}? 非贪婪，防止中文无空格时过度匹配
#   2. "诉"前后不能是"被"（排除"被诉"被动语态）
#   3. "诉"到"案"之间最多15字（真实案例纠纷描述一般不超过15字）
#   4. "诉"到"案"之间不能含句号/分号（防止跨越句子边界）
NAMED_CASE_PATTERN = re.compile(
    r"(?<!被)\S{2,}?诉(?!行为|称|讼)\S{2,}?[^。！？；]{0,15}?(?:纠纷)?案"
)
